<!-- 
  created: 장형욱 
-->
<template>
  <BaseForm :formContext="formContext">
    <div class="row">
      <div class="col-6">
        <FormItem label="배차장소명" :required="true" name="alntPlcNm">
          <Input type="text" v-model="formContext.formData.alntPlcNm"></Input>
        </FormItem>
      </div>
      <div class="col-6">
        <FormItem label="사용안함" label-width="6rem" name="useYn">
        <Input type="checkbox" v-model="formContext.formData.useYn" true-value="N" false-value="Y"></Input>
      </FormItem>
      </div>
    </div>
    <div class="col-6">
        <FormItem label="지역" :required="true" name="areaSeCode">
          <Input type="select" v-model="formContext.formData.areaSeCode" groupCode="CUST019"></Input>
        </FormItem>
      </div>
      <div class="col-6">
        <FormItem label="차고지" :required="true" name="garageSn">
          <Input type="select" :dataSource="garageList" v-model="formContext.formData.garageSn" v-slot="{ item }">
            <el-option :value="item.garageSn" :label="item.garageNm"></el-option>
          </Input>
        </FormItem>
      </div>
      <div class = "col-4">
      <FormItem label="배/회차료" name="alntRetuncarChrge">
        <Input type = "text" v-model="formContext.formData.alntRetuncarChrge"></Input>
      </FormItem>
    </div>
      <div class = "col-8">
      <FormItem label="비고" name="etcContents">
        <Input type = "text"  v-model="formContext.formData.etcContents"></Input>
      </FormItem>
    </div>
  </BaseForm>
</template>

<script lang="ts" setup>
  import { inject, reactive } from 'vue'
  import BaseForm, { IFormContext } from '@/components/reborn/BaseForm.vue'
  import ApiService from '@/core/services/ApiService'
  import FormItem from "@/components/reborn/FormItem.vue";
  import Input from '@/components/reborn/Input.vue'
  import { FormRules } from 'element-plus'

  const props = defineProps({
    garageList: {
      type: Object as () => Array<any>
    }
  })
 
  const reload = inject('reload', () => {})

  const models = reactive<any>({
    garageList: []
  })

  const formContext = reactive<IFormContext>({
    formData: {},
    rules: reactive<FormRules>({
      alntPlcNm: [{ validator: (rule, value, callback) => methods.alntPlcNmChecker(value, callback), trigger: 'blur'}]

    }),
    modal: {
      title: '배차장소',
      width: '800px',
      onOpen(data) {
        formContext.formData = data ?? {}
        ApiService.get('alntPlc').then(({data}) => {
          models.garageList = data.models['garageList']
        })
      }
    },
    addClick(row) {
      ApiService.alert((axios, callback) => axios.post('alntPlc', row).then(callback), {
        label: '등록',
          onSuccess() {
            formContext.modal?.close?.()
            reload()
          }
      })
    },
    updateClick(row) {
      ApiService.alert((axios, callback) => axios.put('alntPlc', row).then(callback), {
        label: '수정',
        onSuccess() {
          formContext.modal?.close?.()
          reload()
        }
    })
  },
  deleteClick(row) {
    ApiService.confirm((axios, callback) => axios.delete('alntPlc', { data:row }).then(callback), {
      label: '삭제',
      onSuccess() {
        formContext.modal?.close?.()
        reload()
      }
    })
  },
  exportClick(row, callBack: any) {
  },
})

const methods = {
  alntPlcNmChecker(value, callbacK) {
    ApiService.call((axios, cb) => axios.get('alntPlc/alntPlcNmCheck', { params: { alntPlcNm: value }}).then(cb), {
      onSuccess(data) {
        const isOverlap = data.data ?? false
        if (isOverlap) {
          callbacK(new Error('배차장소명이 중복되었습니다.'))
        } else {
          callbacK()
        }
      }
    })
  }
}

defineExpose({
  formContext: formContext,
})
</script>

<style>
</style>