        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            <tr v-for="row in models.bcncGradCodeGroups" :key="row">
              <td>
                <Input type="label" v-text="row.first" groupCode="CUST002"></Input>
              </td>
              <td>
                {{ row.second.length }}
              </td>
...
<script lang="ts" setup>
  ...
  const listContext = reactive<IListContext>({
    ...
    onSearch(search?: ISearchFilter) {
      const params = { search: search }
      ApiService.get("bcnc/search", { params: params }).then(({data}) => {            
        listContext.listData = data.data
        models.bcncGradCodeGroups = data.models['bcncGradCodeGroups']
      })
    },
    onReload(search?: ISearchFilter) {
      const params = { search: search }
      ApiService.get("bcnc", { params: params }).then(({data}) => {
        listContext.listData = data.data
        models.bcncGradCodeGroups = data.models['bcncGradCodeGroups']
