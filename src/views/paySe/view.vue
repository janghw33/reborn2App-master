...
<template>
  <BaseForm :formContext="formContext">
    ...
    <div class="col-4">
      <FormItem label="사용안함" name="useYn">
        <Input type="checkbox" v-model="formContext.formData.useYn" true-value="N" false-value="Y"></Input>
      </FormItem>
    </div>
    <div class="col-3">
      <FormItem label="키오스크 결제 여부" name="kioskPayYn" labelWidth="12rem">
...
const formContext = reactive<IFormContext>({
  ...
  updateClick(row) {
    ApiService.alert((axios, callback) => axios.put('paySe', row).then(callback), {
      label: '수정',
      onSuccess() {
        formContext.modal?.close?.()
        reload()
