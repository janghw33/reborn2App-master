    ...
    <div class="row">
      <div class="col-3">
        <FormItem label="거래처명" name="bcncNm" :required="true">
          <Input type="text" :required="true" v-model="formContext.formData.bcncNm" />
        </FormItem>
      </div>
      <div class="col-4">
        <div class="row">
          <div class="col-3">
            <Input type="checkbox" label="거래중지" v-model="formContext.formData.useYn" true-value="N" false-value="Y" />
          </div>
          <div class="col-6">
            <Input type="checkbox" label="에이전트 사용중지" v-model="formContext.formData.agentUseStpgeYn" true-value="Y" false-value="N" />
          </div>
        </div>
      </div>
      <div class="col">
        <FormItem label="거래처 구분" name="bcncSeCode" :required="true">
          <Input type="radio" v-model="formContext.formData.bcncSeCode" groupCode="SYSM009" />
        </FormItem>
      </div>
    </div>    
    <div class="row">
...
<script lang="ts" setup>
  ...
  interface Props {
    insrncChrgeList: Array<any>,
    lendChargeList: Array<any>
  }
  ...
  const formContext = reactive<IFormContext>({
    ...
    addClick(row) {
      ApiService.alert((axios, callback) => {
        axios.post('bcnc', row).then(callback)
      }, {
        label: '등록',
        onSuccess() {
    ...
    updateClick(row) {
      ApiService.alert((axios, callback) => {
        axios.put('bcnc', row).then(callback)
      }, {
        label: '수정',
        onSuccess() {
          formContext.modal?.close?.()
  ...
  watchEffect(() => {
    ...
    formContext.formData.agentUseStpgeYn ??= 'N'
  })
