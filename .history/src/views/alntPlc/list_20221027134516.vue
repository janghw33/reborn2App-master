...
<script lang="ts" setup>
  ...
  const listContext = reactive<IListContext>({
    listData: [],
    headers: [
      { name: "배차장소코드", key: "alntPlcCode", width: 200, sortable: true },
      { name: "배차장소명", key: "alntPlcNm", width: 200, sortable: true },
      { name: "지역", key: "areaSeCode", width: 160, sortable: true, groupCode: 'CUST019' },
      { name: "차고지", key: "garageNm", width: 160, sortable: true },
      { name: "비고", key: "etcContents", sortable: true },
      { name: "배/회차료", key: "alntRetuncarChrge", width: 120, sortable: true },
      { name: "사용여부", key: "useYnTxt", width: 40, sortable: true }
    ],
    perPage: 17,
    search: {
      types: [],
    ...
    onReload(searchFilter?: ISearchFilter) {
      ...
      ApiService.call((axios, callback) => axios.get('alntPlc', {params: params}).then(callback), {
        onSuccess(data) {
          ...
          models.garageNmList = data.models['garageRows']
        }
      })
    },
    onListView(row) {
