...
<template>
  <div>
    <BaseList :context="context" @modal-open="view?.modalOpen">
      ...
      <template v-slot:cell-alntPlcCode="{ row }">
        {{ row.alntPlcCode }}
      </template>
      <template v-slot:cell-garageSn="{ row }">
        {{ row.garageSn }}
      </template>
      <template v-slot:cell-alntPlcNm="{ row }">
        {{ row.alntPlcNm }}
      </template>
      <template v-slot:cell-areaSeCode="{ row }">
        <RebornControl type="label" name="areaSeCode" v-text="row.areaSeCode" groupCode="CUST019" :getCode="(v) => row.areaSeCodeTxt = v"></RebornControl>
      </template>
      <template v-slot:cell-garageNm="{ row }">
        {{ row.garageNm }}
      </template>
      <template v-slot:cell-alntRetuncarChrge="{ row }">
        {{ row.alntRetuncarChrge }}
      </template>
