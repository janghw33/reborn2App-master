  ...
  <BasePage :pageContext="pageContext">
    <BaseList :listContext="listContext">
      <template v-slot:cell-deviceIdTxt="{ row }">
        {{ row.deviceIdTxt }}
        <button v-if="row.deviceId != null " type="submit" @click="deAuth">해제</button>
      </template>
    </BaseList>
    <View ref="view"></View>
  </BasePage>
...
const listContext = reactive<IListContext>({
  ...
  headers: [
    ...
    { name: "단말기 인증 번호",  width: 200, key: "trmnlCrtfcNo", sortable: true },
    { name: "단말기 명", key: "trmnlNm", width: 240, sortable: true },
    { name: "단말기 설명 내용", key: "trmnlDcContents",  sortable: true },
    { name: "사용여부", key: "useYnTxt", width: 160, sortable: true },
    { name: "인증 상태", key: "deviceIdTxt", width: 160, sortable: true },
  ],
  perPage: 17,
  itemBinding(row) {
  ...
  reSearch(searchFilter: ISearchFilter) {
    listContext.listData = models.data.filter(f => {
      return (f.trmnlNm?.indexOf(searchFilter.searchWord) >= 0 ||
      f.trmnlDcContents?.indexOf(searchFilter.searchWord) >= 0 ||
        f.useYnTxt?.indexOf(searchFilter.searchWord) >= 0 ||
        f.deviceIdTxt?.indexOf(searchFilter.searchWord) >= 0) 
    })
  },
...
function deAuth(row) {
  ApiService.alert((axios, callback) => axios.put('trmnlCrtfc/deAuth/{trmnlCode}').then(callback), {
    label: '인증해제',
    onSuccess() {}
  })
}
