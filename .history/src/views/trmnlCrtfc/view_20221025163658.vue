  ...
  <BaseForm :formContext="formContext">
    ...
    <div class ="col-8">
      <FormItem label="단말기설명" :required="true" name="trmnlDcContents">
        <Input type="text" v-model="formContext.formData.trmnlDcContents"></Input>
      </FormItem>
      <FormItem label="사용중" :required="true" name="useYn">
...
<script lang="ts" setup>
  ...
  const formContext = reactive<IFormContext>({
    ...
    addClick(row) {
      ApiService.alert((axios, callback) => axios.post('trmnlCrtfc', row).then(callback), {
        label: '등록',
        onSuccess() {
    ...
    updateClick(row) {
      ApiService.alert((axios, callback) => axios.put('trmnlCrtfc', row).then(callback), {
        label: '수정',
        onSuccess() {
          formContext.modal?.close?.()
          reload()
    ...
    deleteClick(row) {
      ApiService.alert((axios, callback) => axios.delete('trmnlCrtfc', {data:row}).then(callback), {
        label: '삭제',
        onSuccess() {
          formContext.modal?.close?.()
          reload()
