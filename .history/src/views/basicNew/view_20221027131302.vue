<template>
  <BasePage :pageContext="pageContext">
    <BaseForm :formContext="formContext">
      ...
      <div class="row">
        <div class="col-3">
          <FormItem label="단기대여 요금유형" name="lendChrgeClSn" :required="true" labelWidth="140px">1
            <Input type="select" :dataSource="models.lendChargeList" v-model="formContext.formData.lendChrgeClSn" v-slot="{ item }">
        ...
        <div class="col-3">
          <FormItem label="단기보험 요금유형" name="insrncChrgeClSn" :required="true" labelWidth="140px">
            <Input type="select" :dataSource="models.insrncChrgeList" v-model="formContext.formData.insrncChrgeClSn" v-slot="{ item }">
        ...
        <div class="col-3">
          <FormItem label="장기대여 요금유형" name="lendChrgeLtClSn" labelWidth="140px">
            <Input type="select" :dataSource="models.lendChargeList" v-model="formContext.formData.lendChrgeLtClSn" v-slot="{ item }">
              <el-option :value="item.lendChrgeClSn" :label="item.lendChrgeClNm"></el-option>
            </Input>
          </FormItem>
        </div>
        <div class="col-3">
          <FormItem label="장기보험 요금유형" name="insrncChrgeLtClSn" labelWidth="140px">
            <Input type="select" :dataSource="models.insrncChrgeList" v-model="formContext.formData.insrncChrgeLtClSn" v-slot="{ item }">
              <el-option :value="item.insrncChrgeClSn" :label="item.insrncChrgeClNm"></el-option>
            </Input>
          </FormItem>
        </div>
      ...
      <div class="row">
        <div class="col-12">
          <FormItem label="비고2">
            <Input type="textarea" v-model="formContext.formData.etcContents2" rows="5">
            </Input>
          </FormItem>
        </div>
      </div>
...
<script lang="ts" setup>
  ...
  import Input from '@/components/rebornNew/Input.vue'
  import FormItem from '@/components/rebornNew/FormItem.vue'
  ...
  const formContext = reactive<IFormContext>({
    ...
    printClick(callback) {
      ...
      callback(printConfig, formContext.formData)
    },
    exportClick(row, callback: any) {
      
    },
    cancelClick() {
      ApiService.confirm(() => router.push({ path: '/basicNew' }), {
        label: '취소'
      })
    }
  })
