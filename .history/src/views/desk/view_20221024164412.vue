  ...
  <BasePage :pageContext="pageContext">
    <BaseForm :formContext="formContext">
      ...
      <FormItem label="사용안함" label-width="6rem" name="useYn">
        <Input type="checkbox" v-model="formContext.formData.useYn" true-value="Y" false-value="Y"></Input>
      </FormItem>
    </div>
  </div>
      <div class = "col-8">
...
const methods = {
    deskNmChecker(value, callback) {
      ApiService.call((axios, cb) => axios.get('desk/deskNm', { params: { vhctyCode: value } }).then(cb), {
        onSuccess(data) {
