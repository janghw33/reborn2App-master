  ...
  <BaseForm :formContext="formContext">
    ...
    <div class = "col-8">
      <FormItem label="비고" name="etcContents">
        <Input maxlength="200" type = "text" v-model="formContext.formData.etcContents"></Input>
      </FormItem>
    </div>
  </BaseForm>
</template>
...
<script lang="ts" setup>
  ...
  const formContext = reactive<IFormContext>({
    ...
    rules: reactive<FormRules>({
      deskNm: [{ validator: (rule, value, callback) => methods.deskNmChecker(value, callback), trigger: 'change'}]
    }),
    modal: {
      title: '데스크 등록',
      width: '800px',
