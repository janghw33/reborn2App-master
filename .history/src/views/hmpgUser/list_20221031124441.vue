...
<script lang="ts" setup>
 ...
 reSearch(searchFilter: ISearchFilter) {
    listContext.listData = models.data.filter(f => { return (f.userId?.indexOf(searchFilter.searchWord) >=0 || f.userNm?.indexOf(searchFilter.searchWord) >=0 || f.tlphonNoEncpt?.indexOf(searchFilter.searchWord) >=0) })
}
,
