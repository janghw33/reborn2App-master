...
<script lang="ts" setup>
  ...
  const listContext = reactive<IListContext>({
    ...
    headers: [
      ...
      { name: "이름", key: "userNm", width: 120, sortable: true},
      { name: "생년월일", key: "userBrthdy", width: 120, sortable: true },
      { name: "전화번호", key: "tlphonNoEncpt", width: 200, sortable: true },
      { name: "가입일", key: "firstRegistDt", width: 200, sortable: true },
      { name: "마지막 로그인 일시", key: "lastLoginDt", width: 200, sortable: true },
      { name: "면허 종류", :data-source="{'12': '1종 보통', '21': '2종 보통'}" key: "userLcnsKndCode", width: 100, sortable: true  },
    ],
    perPage: 17,
    search: {
      types: [],
  ...
  onReload(searchFilter?: ISearchFilter) {
    ...
    ApiService.call((axios, callback) => axios.get("hmpgUser/search", {params: params}).then(callback), {
      onSuccess(data) {
