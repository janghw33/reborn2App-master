  ...
  <BaseForm :formContext="formContext">
    <div class="col-3">
      <FormItem label="운전자명" :required="true" name="chauferDrverNm">
        <Input
          type="text"
          v-model="formContext.formData.chauferDrverNm"
        ></Input>
      </FormItem>
    </div>
    ...
    <div class="col-3">
      <FormItem label="쇼퍼운전자">
        <Input
          type="checkbox"
          v-model="formContext.formData.innerEmpYn"
          true-value="Y"
          false-value="N"
        ></Input>
      </FormItem>
    ...
    <div class="row">
      <div class="col-5">
        <FormItem label="면허번호" :required="true" name="chauferDrverLcnsNo">
          <Input
            type="text"
            v-model="formContext.formData.chauferDrverLcnsNo"
          ></Input>
        </FormItem>
      </div>
      <div class="col-4">
        <FormItem label="생년월일" :required="true" name="chauferDrverBrthdy">
          <Input
            type="date"
            v-model="formContext.formData.chauferDrverBrthdy"
          ></Input>
        </FormItem>
      </div>
    </div>
    <div class="row">
      <div class="col-5">
        <FormItem
          label="전화번호"
          :required="true"
          name="chauferDrverCttpcEncpt"
        >
          <Input
            type="text"
            v-model="formContext.formData.chauferDrverCttpcEncpt"
          ></Input>
        </FormItem>
      </div>
      <div class="col-4">
        <FormItem
          label="유효일자"
          :required="true"
          name="chauferDrverLcnsValidDe"
        >
          <Input
            type="date"
            v-model="formContext.formData.chauferDrverLcnsValidDe"
          ></Input>
        </FormItem>
      </div>
    </div>
    <div class="col-6">
      <FormItem
        label="면허 종류"
        :required="true"
        name="chauferDrverLcnsKndCode"
      >
        <Input
          type="radio"
          groupCode="SYSM011"
          v-model="formContext.formData.chauferDrverLcnsKndCode"
        ></Input>
      </FormItem>
    </div>
    <div>
      <FormItem label="주소" :rows="3">
        <el-row :gutter="10">
          <el-col :span="16">
            <Input
              type="text"
              v-model="formContext.formData.postNo"
              placeholder="주소검색시 자동입력"
              :readonly="true"
            ></Input>
          </el-col>
          <el-col :span="8">
            <el-button type="primary" @click="methods.findAddress"
              >주소검색</el-button
            >
          </el-col>
          <el-col :span="24">
            <Input
              type="text"
              v-model="formContext.formData.bassAdres"
              placeholder="기본주소"
              :readonly="true"
            ></Input>
          </el-col>
          <el-col :span="24">
            <Input
              type="text"
              v-model="formContext.formData.detailAdres"
              placeholder="상세주소"
            ></Input>
          </el-col>
        </el-row>
...
const formContext = reactive<IFormContext>({
  ...
  addClick(row) {
    ApiService.alert(
      (axios, callback) => axios.post('chauferDrver', row).then(callback),
      {
        label: '등록',
        onSuccess() {
  ...
  updateClick(row) {
    ApiService.alert(
      (axios, callback) => axios.put('chauferDrver', row).then(callback),
      {
        label: '수정',
        onSuccess() {
          formContext.modal?.close?.()
