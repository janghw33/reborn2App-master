...
import BaseList, {IListContext, ISearcseList.vue'
...
const listContext = reactive<IListContext>({
  ...
  reSearch(searchFilter: ISearchFilter) {
    listContext.listData = models.data.filter(f => {
      return (f.chauferDrverNm.indexOf(searchFilter.searchWord) >= 0 || f.innerEmpYnTxt.indexOf(searchFilter.searchWord) >= 0)
      })
  },
