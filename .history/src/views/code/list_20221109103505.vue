        ...
        <div class="col-2">
          <div class="card  car-flush">
            <div class="card-body" style="padding: 10px 20px">
              <label style="display: flex ; justify-content: center; font-size:large;">그룹코드 목록</label>
...
import { rowContextKey } from 'element-plus'
  ...
  const listContext = reactive<IListContext>({
    listData: [],
    headers: [
      { name: "코드", key: "code", sortable: true, width: 200 },
      { name: "코드명", key: "codeNm", sortable: true, width: 200 },
      { name: "비고", key: "codeDc", sortable: true, width: 300 },
      { name: "사용 유무", key: "useYn", sortable: true, width: 200, callback:  (row)=> row.useYn == 'N' ? '사용안함' : '사용중' },
    ],
    perPage: 17,
    search: {
      buttons: {
    ...
    reSearch(searchFilter: ISearchFilter) {
      listContext.listData = models.data.filter(f => {
        return (
          f.code?.indexOf(searchFilter.searchWord) >= 0 || 
          f.codeNm?.indexOf(searchFilter.searchWord) >= 0 ||
          f.codeDc?.indexOf(searchFilter.searchWord) >= 0)
      })
    },
    onReload() {
