          ...
          <div class="card  car-flush">
            <div class="card-body" style="padding: 10px 20px">
              ...
              <table class="table align-middle table-row-dashed fs-6 dataTable no-footer" style="cursor:pointer">
                <tr v-for="codeList in models.codeList" :key="codeList" :class="{'active': codeList == models.codeList }">
                  <td class="fw-bolde text-gray-800 text-center" @click="selectGroupCode(codeList)">
                    {{ $comma(codeList.groupNm)}}
                  </td>
                </tr>
              </table>
            </div>
...
<script lang="ts" setup>
  ...
  function selectGroupCode(codeList) {
    ApiService.get(`code/${codeList.groupCode}`).then(({data}) => {
