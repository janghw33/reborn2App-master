...
import BaseList, {
  IListContext,
  ISearchFilter,
} from '@/components/reborn/BaseList.vue'
...
const listContext = reactive<IListContext>({
  ...
  headers: [
    ...
    { name: '비고', key: 'etcContents', width: 400, sortable: true },
    {
      ...
      width: 100,
      sortable: true,
      callback: (row) => (row.useYn == 'Y' ? '사용함' : '사용안함'),
    },
  ],
  perPage: 17,
  search: {
  ...
  reSearch(searchFilter: ISearchFilter) {
    listContext.listData = models.data.filter((f) => {
      console.log(listContext.listData)
      return (
        f.paySeContents.indexOf(searchFilter.searchWord) >= 0 ||
  ...
  onReload(searchFilter?: ISearchFilter) {
    ...
    ApiService.call(
      (axios, callback) =>
        axios.get('paySe', { params: params }).then(callback),
      {
        onSuccess(data) {
