...
<template>
  <BaseForm :formContext="formContext">
    ...
    <div class="col-4">
      <FormItem label="사용안함" name="paySeContents">
        <Input type="checkbox" v-model="formContext.formData.useYn" true-value="N" false-value="N"></Input>
      </FormItem>
    </div>
    <div class="col-3">
      <FormItem label="키오스크 결제 여부" name="paySeContents" labelWidth="12rem">
        <Input type="checkbox" v-model="formContext.formData.kioskPayYn" true-value="Y" false-value="N"></Input>
      </FormItem>
    </div>
    <div class="col-12">
      <FormItem label = "입금방법 형태" :required="true" name="payStleCode">
        <Input type = "radio" group-code="SYSM010" v-model="formContext.formData.payStleCode"></Input>
      </FormItem>
    </div>
    <div class = "col-8">
      <FormItem label="비고">
...
const formContext = reactive<IFormContext>({
  ...
  addClick(row) {
    ApiService.alert((axios, callback) => axios.post('paySe', row).then(callback), {
        label: '등록',
        onSuccess() {
