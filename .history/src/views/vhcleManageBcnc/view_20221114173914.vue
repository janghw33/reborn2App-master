...
import axios from 'axios'
  ...
  const formContext = reactive<IFormContext>({
    ...
    rules: reactive<FormRules>({
      vhcleManageBcncNm: [{ validator: (rule, value, callback) => methods.vhcleManageBcncNmChecker(value, callback), trigger: 'blur'}]
    }),
  modal: {
    title: '차량관리 거래처',
    width: '1000px',
  ...
  addClick(row) {
    ApiService.alert((axios, callback) => axios.post('vhcleManageBcnc', row).then(callback), {
      label: '등록',
        onSuccess() {
  ...
  updateClick(row) {
    ApiService.alert((axios, callback) => {axios.put('vhcleManageBcnc', row).then(callback)
    }, {
      label: '수정',
      onSuccess() {
        formContext.modal?.close?.()
