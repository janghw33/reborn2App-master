...
import { reactifyObject, timestamp } from '@vueuse/shared'
  ...
  const listContext = reactive<IListContext>({
    ...
    { name: "작성시간", key: "acdntReprtSn", width: 120, sortable: true, callback: (row) =>  },
    ],
    perPage: 17,
    search: {
      types: [],
    ...
    onReload(searchFilter?: ISearchFilter) {
      ...
      ApiService.call((axios, callback) => axios.get('hmpgVhcty', {params: params}).then(callback), {
        onSuccess(data) {
          models.insrntList = data.models['insrntList']
