      ...
      <div class="row">
        <div class="col-6">
          <div class="card-body pt-2 pb-2" style="padding: 0px 16px">
            <FormItem label="차종" :required="true" name="vhctyNm">
              <Input
                type="text"
                v-model="formContext.formData.vhctyNm"
                :readonly="true"
              ></Input>
            </FormItem>
          </div>
        ...
        <div class="col-6">
          ...
          <FormItem label="일반보험매핑" name="gnrlInsrncSn">
            <Input
              type="select"
              :dataSource="models.insrncList"
              v-model="formContext.formData.gnrlInsrncSn"
            >
              <template type="select" v-slot="{ item }">
                <el-option
                  :value="item.insrncKndSn"
                  :label="item.insrncKndNm"
                ></el-option>
              </template>
            </Input>
          </FormItem>
          <FormItem label="완전보험매핑" name="prfectInsrncSn">
            <Input
              type="select"
              :dataSource="models.insrncList"
              v-model="formContext.formData.prfectInsrncSn"
            >
          ...
          <FormItem label="프리미엄 여부" name="deskNm">
            <Input
              type="checkbox"
              v-model="formContext.formData.expsrGradCode"
              true-value="P"
              false-value=""
            ></Input>
          </FormItem>
          <FormItem label="추천여부" :required="true" name="recomendYnTxt">
            <Input
              type="checkbox"
              v-model="formContext.formData.recomendYn"
              true-value="N"
              false-value="Y"
            ></Input>
          </FormItem>
...
const formContext = reactive<IFormContext>({
  ...
  updateClick(row) {
    ApiService.alert((axios, callbakc) => axios.put('hmpgVhcty', row).then(callback), {
      label: '수정',
      onSuccess() {
  ...
  deleteClick(row) {
    ApiService.confirm((axios, callback) => axios.delete('hmpgVhcty', { data: row}).then(callback), {
      label: '삭제',
      onSuccess() {
        formContext.modal?.close?.()
        reload()
