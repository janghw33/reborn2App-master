...
<template>
  ...
  <BasePage :pageContext="pageContext">
    <BaseList :listContext="listContext">
      <template :dataSource="models.insrncList" v-model:insrncList.gnrlInsrncSn v-slot:cell-gnrlInsrnSn="{insrncList}">
        <el-option :value="insrncList.insrncKndSn" :label="insrncList.insrncKndNm"></el-option>
      </template>
...
<script lang="ts" setup>
  ...
  const listContext = reactive<IListContext>({
    ...
    { name: "프리미엄 여부", key: "expsrGradCodeTxt", width: 120, sortable: true,  },
    ...
    itemBinding(row) {
      row.recomendYnTxt = row.recomendYn == 'Y' ? '추천' : '비추천'
      row.sleYnTxt = row.sleYn == 'Y' ? '판매' : '비판매'
      row.expsrGradCodeTxt = row.expsrGradCode == 'P' ? '이용' : '미이용'
    ...
    reSearch(searchFilter: ISearchFilter) {
      listContext.listData = models.data.filter(f => {
        return (
          ...
          f.sleYnTxT?.indexOf(searchFilter.searchWord) >= 0 ||
          f.expsrGradCodeTxt?.indexOf(searchFilter.searchWord) >= 0
        )
      })
    },
